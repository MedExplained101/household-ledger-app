...
- `confidence`: free text for now, e.g. `receipt` / `web-search` / `estimate` / `in-store-scan` (added 2026-08-15 — see below)
- `confidence: in-store-scan` — added 2026-08-15 for the in-store price logging feature (`log_price` webhook action): a human reading a real shelf price and confirming it via a barcode scan is treated as higher-confidence than a web scrape, so it writes straight to `Catalog` immediately instead of sitting through the Price Refresh Agent's 24–48h freshness gate.
...
- `status: confirmed` — added 2026-08-15 for the `log_price` action: a human-confirmed in-store price bypasses the freshness-gate `pending` state entirely and is logged straight as `confirmed`, since it never needs promoting from anywhere.
...
- `lat`/`long` can now also be populated from the app itself — added 2026-08-15 for the "Manage stores" feature (`update_store_location` webhook action): a "Use my location" button in-app reads the phone's GPS position while physically standing at the store and writes `lat`/`long` directly, instead of requiring manual entry. `address` is left untouched by this action in the v1 scaffold (see TODO.md for the deferred reverse-geocode follow-up) — same "only fill blank fields, never overwrite a manual entry" convention already used for `est_trip_cost`/`product_url`.
