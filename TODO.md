...
- [ ] Decide on and document one canonical deploy path (Netlify vs Vercel) — the README currently presents both as equally valid, which can lead to drift between two live deployments. **Note (2026-07-30):** `netlify.toml` already exists and Netlify is already auto-previewing every PR on this repo (see the deploy-preview bot comments) — this may already be settled by infrastructure rather than still open. Flagged for Graham to confirm before the README is rewritten to state Netlify as primary.
