...
- [ ] Recommendation Agent — bulk-trip economics (is a farther bulk-price store worth the trip, based on `Inventory.avg_days_to_finish` × price gap vs. `est_trip_cost`). Not started; gated behind 8 completed purchase cycles per item before being called "reliable" rather than "provisional." No scaffold drafted yet — depends on the `Stores` tab and real `Inventory` history existing first.
